...
- [ ] **LangChain.js handler** — `BudgetGuardHandler extends BaseCallbackHandler`: `handleLLMEnd` meters (prefer `generations[0][0].message.usage_metadata` = `{input_tokens,output_tokens}`, fall back to `llmOutput.tokenUsage` = `{promptTokens,completionTokens}` — never both), `handleChatModelStart`/`handleLLMStart` throws pre-call over cap. Test: both usage shapes present → recorded once; over-cap start → throws.
  - **Research notes (2026-07-05):** needs a **subpath export `budget-guard/langchain`** + `@langchain/core` as an OPTIONAL peer dep (must extend `BaseCallbackHandler`, so core stays zero-dep only for the main entry). Model name is provider-specific: read `generations[0][0].message.response_metadata.model` / `.model_name` / `llmOutput.model_name`, prefer an explicit `opts.model` (date-suffixed ids like `gpt-4o-2024-..` won't match PRICES). START cap check needs a standalone "total spent today" (add exported `spentTotal(project, store?, day?)`); recording can reuse `guard()` with `dailyCapUSD: Infinity` (pure meter). Same pattern will apply to LlamaIndex/Mastra.

...
- [ ] **`examples/` directory** — runnable examples: basic cap + spendReport, **Redis shared-store worker fleet** (lands @raju_dandigam's "much more useful for worker fleets" note — the Redis store shipped in v0.2 *after* the launch article, so make it visible), and `onSpend` cost-observability.

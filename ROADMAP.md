...
- [x] **Vercel AI SDK v5 middleware** — `budgetGuardMiddleware()` returning a `LanguageModelV2Middleware`-shaped object (structurally typed, zero dep); `wrapGenerate` does the pre-call cap throw + post-call meter by **reusing `guard()`** (no billing logic duplicated). Used via `wrapLanguageModel`. Verified against real `ai@5`: over-cap call throws before `doGenerate` runs (spy). (`streamText`/`wrapStream` is the next item.)
